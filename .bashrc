...
__arduino-cli_get_completion_results() {
    ...
    # Calling ${words[0]} instead of directly arduino-cli allows handling aliases
    args=("${words[@]:1}")
...
__arduino-cli_process_completion_results() {
    ...
    __arduino-cli_handle_activeHelp
}
...
__arduino-cli_handle_activeHelp() {
    ...
    if ((${#activeHelp[*]} != 0)); then
        if [ -z $COMP_TYPE ]; then
            ...
            printf "%s\n" "${activeHelp[@]}"
            printf "\n"
            __arduino-cli_reprint_commandLine
            return
        fi
        ...
        if [ $COMP_TYPE -eq 63 ]; then
            ...
            if ((${#COMPREPLY[*]} == 0)); then
                ...
                if (((directive & shellCompDirectiveNoFileComp) == 0)); then
                    __arduino-cli_debug "Listing files"
                    _filedir
                fi
            fi
...
__arduino-cli_handle_standard_completion_case() {
    ...
    if ((${#COMPREPLY[*]} == 1)); then
        ...
        __arduino-cli_debug "Removed description from single completion, which is now: ${COMPREPLY[0]}"
    else
