...
# 4. Check if systemd has already been manually configured
if grep -q "SSL_CERT_FILE=/etc/ssl/certs/ca-certificates.crt" /etc/systemd/system.conf 2>/dev/null; then
  echo " Systemd background services are already configured."
